...
//        G_clientConnectionType = CLIENT_CONNECTION_TYPE_DIRECT_IPX ;
    ...
    ClientSyncSetNumberPlayers(numPlayers) ;
...
//printf("OSO: %d %d %d %d\n", objectType, sourceObject, angle, velocity) ;
    ...
    /** ... with speed 50 ... **/
    p_addPacket->initialSpeed = velocity ;
...
T_void ClientUpdate(T_void)
{
    ...
    if ((G_clientIsActive == TRUE) && (G_logoutAttempted == FALSE))  {
        if (G_clientMode == CLIENT_MODE_GAME)  {
            ...
            if ((KeyMapGetScan(KEYMAP_JUMP)==TRUE) &&
                 (G_msgOn == FALSE))  {
                /* Do a jump manuever. */
                if (!ClientIsDead())
                ...
                (EffectPlayerEffectIsActive(PLAYER_EFFECT_FLY)==TRUE))
		    {
		        ...
		        if ((timeHeld) && ((rand() % 5)==0))  {
			        PlayerAccelDirection(playerMoveAngle, 1) ;
                    PlayerSetStance(STANCE_WALK) ;
                }
                timeHeld2 = KeyMapGetHeld(KEYMAP_BACKWARD) ;
...
/****************************************************************/
    if (ClientIsInView())
    {
                ...
                if (G_msgOn==FALSE)
                {
				    ...
				    if (scankey == KEY_SCAN_CODE_T)
                    {
					     if (KeyboardGetScanCode(KEY_SCAN_CODE_ALT)==TRUE)
                         {
                             ...
                             fscanf(fp, "%d %d %u", &teleportX, &teleportY, &teleportAngle) ;
                             fclose(fp) ;
                             PlayerTeleport(teleportX, teleportY, teleportAngle) ;
				    ...
				    if (scankey == KEY_SCAN_CODE_S)
                    {
					     if (KeyboardGetScanCode(KEY_SCAN_CODE_ALT)==TRUE)
                         {
                             ...
                             fprintf(fp, "%d %d %u", PlayerGetX16(), PlayerGetY16(), PlayerGetAngle()) ;
                             fclose(fp) ;
                    ...
                    {
                        StatsSetCompletedAdventure(StatsGetCompletedAdventure()+1);
                        MessagePrintf ("LastAdventureCompleted=%d",StatsGetCompletedAdventure());
                    }
                }
                break;
        ...
        switch (event)
        {
            ...
            if (G_msgOn == TRUE)
            {
				...
				} else if ((scankey == '\r') || (scankey == '\n'))
                {
                    ...
                    if (ClientIsGod())
                    {
                        ...
                        {
                            sscanf(G_message+5, "%d%d%d%d", &x, &y, &height, &angle) ;
                        ...
                        {
                            char buf[40];
                            sprintf (buf, "Location: %d, %d, %d", PlayerGetX16 (),
                                     PlayerGetY16 (), PlayerGetZ16 ());
                            MessageAdd (buf);
                        }
                        else if (strncmp(G_message, "@blam", 5) == 0)
                        {
                            MessageAdd("Creatures destroyed") ;
                            MessagePrintf("Total XP: %ld", CreaturesKillAll()) ;
                        }
                        ...
                        else if (strncmp (G_message, "@create", 7) == 0)
                        {
                            ...
                            {
                                MessageAdd("Can't create while item in hand.") ;
                            }
                        }
            ...
            if (KeyboardGetScanCode(KEY_SCAN_CODE_ALT)==TRUE)  {
                if ((KeyboardGetScanCode(KEY_SCAN_CODE_F6)==TRUE))  {
                    MessageAdd("Dumping sync memory") ;
                    SyncMemDump() ;
                }
            }
#endif
			...
			if ((KeyMapGetScan(KEYMAP_ABORT_LEVEL)==TRUE)
                           && (!BannerIsOpen()))  {
                if (G_escCount==1)  {
...
{
    T_packetShort packet ;
    T_specialEffectPacket *p_packet ;
