...
//#undef DebugRoutine
//#        define DebugRoutine(name) DebugAddRoutine(name, __FILE__, __LINE__) ; puts(name)
...
//static T_3dObject* ControlThrowObject (T_3dObject *p_object, T_word16 x, T_word16 y);
/* LES changed: */
static T_void ControlExamineObject (T_3dObject *p_obj);
static T_word16 ControlSetMovePointer (T_word16 mx, T_word16 my);
static T_void ControlColorizeLookString (T_byte8 *string);
...
//    }
    ...
    if (jumpTimer > JUMP_RESET_TIME) jumpTimer=0;
    ...
    switch (event)
    {
        ...
        if (button==MOUSE_BUTTON_LEFT)
        {
            ...
            if (G_mouse.mode==CONTROL_MOUSE_MODE_NORMAL)
            {
                if (InventoryObjectIsInMouseHand())
                {
                    ControlSetObjectPointer (InventoryCheckObjectInMouseHand());
                } else
                {
...
//            }
        ...
        {
            if (InventoryCanUseItemInReadyHand()) InventoryUseItemInReadyHand(NULL);
        }
        ...
        switch (G_mouse.mode)
        {
            ...
            G_lookAngle = (((VIEW3D_CLIP_RIGHT-VIEW3D_CLIP_LEFT+1)/2) - x) << 6;
            /* check for a spell cast command *left+right buttons* */
            if (button==3 && G_resetNeeded==FALSE && HardFormIsOpen()==FALSE)
            {
                if (ViewIsAt(x,y))
                {
                    /* cast a spell */
            ...
            if ((dir == 0) || (dir == 1) || (dir == 7))
                PlayerMoveForward(((vcy-y)<<10)/(VIEW3D_HEIGHT/2)) ;
            if ((dir >= 3) && (dir <= 5))
                PlayerMoveBackward(((y-vcy)<<10)/(VIEW3D_HEIGHT/2)) ;
...
//    }
    ...
    /* figure quadrant */
    if      (y<= dx && y<=-dx) qnum=0;
    else if (y> -dx && y<=-hx) qnum=1;
    else if (y> -hx && y<= hx) qnum=2;
    else if (y>  hx && y<= dx) qnum=3;
    else if (y>  dx && y> -dx) qnum=4;
    else if (y<=-dx && y> -hx) qnum=5;
    else if (y<=-hx && y>  hx) qnum=6;
    else qnum=7;
...
//                    if (size > 2) desc2[size-2]='\0';
        ...
        if (p_obj)  {
            ...
            if ((ObjectIsCreature(p_obj)==TRUE) && (!CreatureIsMissile(p_obj)))
            {
...
//                LookRequestPlayerInfo (p_obj);
                MessagePrintf(
                    "^011Player ^009%s",
                    PeopleHereGetPlayerIDName(ObjectGetServerId(p_obj)-9000)) ;
            }
        }
    } else
    {
