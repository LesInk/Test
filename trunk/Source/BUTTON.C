...
static T_buttonID ButtonInit (T_word16 lx,
										T_word16 ly,
										T_byte8 *bmname,
					  ...
					  T_buttonHandler p_cbroutine2)
{
	...
	for (i=0;i<MAX_BUTTONS;i++)
	{
		...
		{
			G_buttonarray[i]=ButtonInit(lx,ly,bmname,toggletype,keyassoc,p_cbroutine,p_cbroutine2);
			break;
		}
	}
	...
	if (buttonID!=NULL)
	{
		for (i=0;i<MAX_BUTTONS;i++)
		{
			if (G_buttonarray[i]==buttonID) //found it, now kill it
			{
				...
				if (p_button->textID!=NULL) TextDelete (p_button->textID);
		...
		else
		{
			for (i=0;i<MAX_BUTTONS;i++)
			{
				if (G_buttonarray[i]!=NULL)
				{
                    ...
                    DebugCheck (p_button->tag==BUTTON_TAG);
					keyscan=p_button->scancode;
					if ((keyscan>>8) && (KeyboardGetScanCode (KEY_SCAN_CODE_ALT)==TRUE))
					{
						keyscan&=0xFF;
						if (keyscan==scankey)
						{
							...
							{
								if (p_button->pushed==FALSE) ButtonDown (p_button);
								else
								{
									ButtonUp (p_button);
								}
							} else ButtonDown (p_button);
						}
					}
					else if ((keyscan==scankey && keyscan>0) && (KeyboardGetScanCode (KEY_SCAN_CODE_ALT)==FALSE))
					{
						if (p_button->toggle==TRUE)
						{
							if (p_button->pushed==FALSE) ButtonDown (p_button);
			...
			for (i=0;i<MAX_BUTTONS;i++)
			{
				if (G_buttonarray[i]!=NULL)
				{
					...
					if (p_button->toggle==FALSE)
					{
						...
						else if ((keyscan==scankey) && (KeyboardGetScanCode (KEY_SCAN_CODE_ALT)==FALSE))
						{
							ButtonUp (p_button);
						}
					}
...

{
    ...
    if (!ButtonIsValid (buttonpushed)) buttonpushed=NULL;

	if (buttonpushed != NULL) p_button = (T_buttonStruct *)buttonpushed;
	...
	switch (event)
	{
        ...
        if (button==MOUSE_BUTTON_LEFT) buttonpushed=G_mouseOverButton;
        else buttonpushed=NULL;
		...
		if (buttonpushed != NULL)
        {
            ...
            if (p_button->toggle==FALSE)
		    {
			    ...
			    {                                      /* was moved off of button */
				    if (p_button->pushed==TRUE) ButtonUpNoAction (buttonpushed);
			    }
            }
		}
		break;
...
T_void ButtonDownNum(T_word16 num)
{
    ...
    DebugCheck (p_button->tag==BUTTON_TAG);
	if (p_button != NULL) ButtonDownNoAction (G_buttonarray[num]);
	DebugEnd();
}
...
T_void ButtonUpNum(T_word16 num)
{
	...
	p_button = (T_buttonStruct *)G_buttonarray[num] ;
    DebugCheck (p_button->tag==BUTTON_TAG);
	if (p_button != NULL) ButtonUpNoAction (G_buttonarray[num]);
	DebugEnd();
}
...
T_void ButtonDoCallback2 (T_buttonID buttonID)
{
	...
	if (p_button->p_callback2 != NULL) p_button->p_callback2(buttonID);
...
T_void ButtonSetText (T_buttonID buttonID, T_byte8 *string, T_byte8 color)
{
	T_buttonStruct *p_button;
	T_textStruct *p_text;
	...
	{
		p_button->textID=TextCreate (p_buttongraphic->locx,p_buttongraphic->locy,string);
...
//	DebugCheck (p_textgraphic->width<p_buttongraphic->width);
	...
	textx=p_buttongraphic->locx+(p_buttongraphic->width/2)-(p_textgraphic->width/2);
	texty=p_buttongraphic->locy+(p_buttongraphic->height/2)-(p_textgraphic->height/2);
...
E_Boolean ButtonIsMouseOverButton (T_void)
{
    ...
    if (G_mouseOverButton != NULL) retvalue=TRUE;
