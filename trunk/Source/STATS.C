...
//              ColorAddGlobal (0,0,amt>>3);
    ...
    //    fflush (stdout);
        ...
        if (effects>0)
        {
            ...
            if (resistpct > 0.8) resistpct=0.8;
...
T_void StatsChangePlayerExperience (T_sword32 amt)
{
    ...
    else
    {
        ...
        amt = -amt;
        if (amt > G_activeStats->Experience) amt = G_activeStats->Experience;
...
T_void StatsToggleGodMode (T_void)
{
    ...
    if (EffectPlayerEffectIsActive (PLAYER_EFFECT_GOD_MODE)==TRUE)
    {
        ...
        for (i=0;i<9;i++) StatsDecrementRuneCount (i);
    }
    else
    {
        /* add god mode effect */
        ...
        for (i=0;i<9;i++) StatsIncrementRuneCount (i);
...
T_statsSavedCharacterID* StatsGetSavedCharacterIDStruct (T_byte8 selected)
{
