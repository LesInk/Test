...
static T_formObjectID FormCreateObject   (E_formObjectType type,
	...
	for (i=0;i<MAX_FORM_OBJECTS;i++)
	{
		...
		if (p_object->objtype==FORM_OBJECT_BUTTON)
		{
			if ((T_buttonID)p_object->objID==buttonID)
			{
				...
				{
					formcallback(FORM_OBJECT_BUTTON,buttonAction,p_object->numID);
					/* leave the loop */
					break;
				}
			}
   ...
   T_word32 size;
	...
	while (feof(fp)==FALSE)
	{
        ...
        if (tempstr[0]=='$')
        {
            ...
            for (i=1;i<strlen(tempstr);i++) tempstr2[i-1]=tempstr[i];
            tempstr2[i-1]='\0';
		...
		if (tempstr[0]!='#' && tempstr[0]!=' ')
		{
			...
			} else if (objtype==3) /* add a button */
			{
				sscanf (tempstr,"%d,%d,%d,%d,%d,%d,%s",
						  ...
						  &hotkey,
						  picname);
 				FormAddButton (x1,y1,picname,(E_Boolean)toggletype,hotkey,objid);
			} else if (objtype==4) /* add a text button */
			{
				...
				if (tempstr[strlen(tempstr)-1]=='\n') tempstr[strlen(tempstr)-1]='\0';
				/* make a text button */
				FormAddTextButton (x1,y1,tempstr,picname,fontname,
										 fcolor,0,(E_Boolean)toggletype,hotkey,
										 objid);
			}
...
//                fgets (tempstr,128,fp);
            ...
            if (sbupID != 0)
            {
               ...
               TxtboxSetScrollBarObjIDs(p_obj->objID,SBUbuttonID,SBDbuttonID,SBGgraphicID);
            }
								...
								E_Boolean button)
{
   ...
   if (G_formHasTextBoxes==TRUE) TxtboxMouseControl (event,x,y,button);
...
T_void FormHandleKey (E_keyboardEvent event, T_word16 scankey)
{
    ...
    if (G_formHasButtons==TRUE) ButtonKeyControl (event,scankey);
    if (G_formHasTextBoxes==TRUE) TxtboxKeyControl (event,scankey);
...
//        MouseShow();
//        delay (20);
	} while (*exitflag==FALSE && KeyboardGetScanCode(KEY_SCAN_CODE_ESC)==FALSE);
