...
static E_Boolean G_songIsPlaying = FALSE ;
...
static T_byte8 G_musicBuffer1[SIZE_SOUND_STREAM_BUFFER] ;
static T_word16 G_musicBuffer1Len = 0 ;
static T_byte8 G_musicBuffer2[SIZE_SOUND_STREAM_BUFFER] ;
static T_word16 G_musicBuffer2Len = 0 ;
...
static T_void ICheckForNextMusicUpdate(T_void)
{
    if (G_musicNeedsUpdate)  {
        ...
        if (G_nextMusicBuffer != 0)  {
            G_musicBuffer1Len = FileRead(G_musicFile, G_musicBuffer1, SIZE_SOUND_STREAM_BUFFER) ;
            G_nextMusicBuffer = 0 ;
        } else {
            G_musicBuffer2Len = FileRead(G_musicFile, G_musicBuffer2, SIZE_SOUND_STREAM_BUFFER) ;
            G_nextMusicBuffer = 1 ;
        }
...
{
    if (G_nextMusicBuffer == 0)  {
        p_sample->pSample = G_musicBuffer1 ;
        p_sample->wLength = G_musicBuffer1Len ;
    } else {
        p_sample->pSample = G_musicBuffer2 ;
        p_sample->wLength = G_musicBuffer2Len ;
    }
...
/* TESTING */
//filename = "d:\\maxdemo\\game_msc.wav" ;
filename = "d:\\cddemo\\roadrash\\audio\\music\\sg_kicks.rra" ;
    if (SoundIsOn())  {
        ...
        if (strcmp(filename, G_currentSong) != 0)  {
            ...
            /* Stop the original song (if playing). */
	        if (G_songIsPlaying)
	        {
                /* stop playing the last song. */
                sosDIGIStopSample(hDIGIDriver, G_musicHandle) ;
            ...
            G_musicFile = FileOpen(filename, FILE_MODE_READ) ;
            if (G_musicFile != FILE_BAD)  {
                G_musicLength = FileGetSize(filename) ;

                G_songIsPlaying = TRUE ;
                G_musicBuffer1Len = FileRead(G_musicFile, G_musicBuffer1, SIZE_SOUND_STREAM_BUFFER) ;
...
T_void SoundSetBackgroundVolume(T_byte8 volume)
{
    ...
    if (G_soundsInit)  {
        sosMIDISetMasterVolume(volume>>1) ;
    }
