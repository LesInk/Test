...
T_void TownUIStart  (T_word32 formNum)
{
    ...
    const T_byte8 menuN[4][6]={"QUEST",
                               "INN",
                               "STORE",
                               "BANK"} ;
    const T_byte8 menuN2[4][6]={"GUILD",
                               "INN",
                               "STORE",
                               "BANK"} ;
    T_word16 menuHotKey[4];

    const T_word16 placeToGo[4]={HARD_FORM_GUILD,
                                 HARD_FORM_INN,
                                 HARD_FORM_STORE,
                                 HARD_FORM_BANK} ;
    const T_word16 placeToGo2[4]={0xFFFF,
                                 HARD_FORM_INN,
                                 HARD_FORM_STORE,
                                 HARD_FORM_BANK} ;
    ...
    for (i=0;i<4 /* 5 */;i++)
    {
        ...
        G_menuButtons[i]=ButtonCreate(menuX[i],
                                      17,
                                      stmp,
                                      FALSE,
                                      menuHotKey[i],
    ...
    if (G_isOnePlayer==FALSE)
    {
        ...
        G_userUpButton=ButtonCreate(63,
                                    35,
                                    "UI/COMMON/UPARROW",
                                    FALSE,
                                    0,
        ...
        G_userDnButton=ButtonCreate(63,
                                    129,
                                    "UI/COMMON/DNARROW",
                                    FALSE,
                                    0,
        ...
        TxtboxSetScrollBarObjIDs(G_userListBox,G_userUpButton,G_userDnButton,G_userScrollBar);
    ...
    else
    {
        ...
        G_userLastButton=ButtonCreate(41,
                                      35,
                                      "UI/TOWN/LAST",
                                      FALSE,
                                      0,
                                      NULL,
                                      TownUILastQuest);
        G_userNextButton=ButtonCreate(188,
                                      35,
                                      "UI/TOWN/NEXT",
                                      FALSE,
                                      0,
        ...
        /* set up scroll buttons */
        G_userUpButton2=ButtonCreate(200,
                                    64,
                                    "UI/COMMON/UPARROW",
                                    FALSE,
                                    0,
                                    NULL,
                                    TownUIDescriptionBoxUp);

        G_userDnButton2=ButtonCreate(200,
                                    129,
                                    "UI/COMMON/DNARROW",
                                    FALSE,
                                    0,
                                    NULL,
                                    TownUIDescriptionBoxDn);
        /* set up scroll bar */
        G_userScrollBar2=GraphicCreate(200,74,"UI/TOWN/SB2");

        /* set up scroll bar control */
        TxtboxSetScrollBarObjIDs(G_descriptionBox,G_userUpButton2,G_userDnButton2,G_userScrollBar2);
...
//    KeyboardSetEventHandler (TownUIHandleKey);
    ...
    if (G_adventureComplete==TRUE)
    {
        ...
        if (G_isOnePlayer) TownUIUpdateQuestInfo();
...
T_void TownUIEnd    (T_void)
{
    ...
    if (G_isOnePlayer==FALSE)
    {
        ...
        /* free message memory */
        for (i=0;i<TOWN_NUM_MESSAGES;i++) MemFree(G_messages[i]);
        KeyboardBufferOff();
    }
    else
    {
...
static T_void TownUIGotoPlace (T_buttonID buttonID)
{
    ...
    if (ButtonGetData(buttonID) == 0xFFFF)  {
        ...
        ClientSetNextPlace(HARDFORM_GOTO_PLACE_OFFSET+ButtonGetData(buttonID),0);
    }
...
T_void TownUIAddMessage (T_byte8 *playerName, T_byte8 *message)
{
    ...
    else
    {
        ...
        /* add message here */
        if (playerName != NULL)
            sprintf (G_messages[TOWN_NUM_MESSAGES-2],"^003%s^007:\r",playerName);
...
static T_void TownUIUpdateQuestInfo (T_void)
{
    ...
    else
    {
        ...
        sprintf (stmp,"^007     Maps Completed:\t^009%d of %d^007\r",lastMapCompleted,nummaps);
        strcat(stmp3,stmp);
    }
...
static E_Boolean TownUIFinishedQuest (T_void)
{
    ...
    {
        if (InventoryHasItem(INVENTORY_PLAYER,questItem)>0) iSucceeded=TRUE;
    }
    ...
    if (iSucceeded==TRUE)
    {
        ...
        if (strcmp(stmp,"FALSE")==0) keepItem=FALSE;
        else keepItem=TRUE;
    ...
    */
            if (rewardExp != 0)
            {
                sprintf (stmp,"^009You have gained ^018%d ^009experience!",rewardExp);
                FormPush() ;
                SoundPlayByNumber(2014,255);
                PromptDisplayMessage(stmp);
                FormPop() ;
...
E_Boolean TownUICompletedMapLevel(T_word16 mapLevel)
{
    ...
    if (G_isOnePlayer)  {
        ...
        sprintf (stmp2,"^003Map %d of ^009%s ^003completed!\r^011~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~\r\r^007%s",i,stmp3,stmp);
